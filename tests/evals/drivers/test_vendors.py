...
def test_parse_behaviours(tmp_path):
    def test_parse_claude_behaviours():
        ...
        test_parse_claude_json_result_usage_and_cost()
        test_parse_claude_json_with_embedded_calls_splits_toolsearch()
        test_parse_claude_json_preserves_error_subtype()

    def test_parse_claude_transcript_calls(tmp_path):
    ...
    def test_parse_codex_behaviours():
        ...
        test_parse_codex_jsonl_events()
        test_parse_codex_jsonl_events_v0147_schema()
        test_parse_codex_jsonl_events_mixed_old_and_new_schema()

    test_parse_claude_behaviours()
    _d1 = tmp_path / "test_parse_claude_transcript_calls"
    _d1.mkdir()
    test_parse_claude_transcript_calls(_d1)
    test_parse_codex_behaviours()


def test_find_behaviours(tmp_path, monkeypatch):
    def test_find_codex_rollout_exact_match_and_unmatched(tmp_path, monkeypatch):
    ...
    def test_find_codex_rollout_session_meta_id(tmp_path, monkeypatch):
        from evals import drivers as drivers_mod
    ...
    _d0 = tmp_path / "test_find_codex_rollout_exact_match_and_unmatched"
    _d0.mkdir()
    with pytest.MonkeyPatch.context() as mp:
        test_find_codex_rollout_exact_match_and_unmatched(_d0, mp)
    _d1 = tmp_path / "test_find_codex_rollout_session_meta_id"
    _d1.mkdir()
    with pytest.MonkeyPatch.context() as mp:
        test_find_codex_rollout_session_meta_id(_d1, mp)


def test_codex_behaviours(tmp_path, monkeypatch):
    def test_codex_driver_notes_rollout_unmatched_when_no_file(tmp_path, monkeypatch):
    ...
    def test_codex_driver_behaviours():
        ...
        test_codex_driver_parses_fake_stdout_no_live()
        test_codex_driver_refuses_live_by_default()

    _d0 = tmp_path / "test_codex_driver_notes_rollout_unmatched_when_no_file"
    _d0.mkdir()
    with pytest.MonkeyPatch.context() as mp:
        test_codex_driver_notes_rollout_unmatched_when_no_file(_d0, mp)
    test_codex_driver_behaviours()
...
def test_claude_behaviours(tmp_path, monkeypatch):
    def test_claude_driver_falls_back_to_transcript(tmp_path, monkeypatch):
    ...
    def test_claude_driver_writes_mcp_config_and_cmd_flags(tmp_path):
        seen: dict[str, Any] = {}
    ...
    def test_claude_driver_server_command_override(tmp_path):
        seen: dict[str, Any] = {}
    ...
    def test_claude_driver_behaviours():
        ...
        test_claude_driver_timeout_returns_agent_run_not_raise()
        test_claude_driver_json_parse_failure_raises_for_infra_cli()

    def test_claude_driver_uses_proxy_in_mcp_config(tmp_path):
        seen: dict = {}
    ...
    def test_claude_driver_proxy_disabled_no_wrap(tmp_path):
        seen: dict = {}
    ...
    def test_claude_mcp_env_has_pythonpath_when_proxied(tmp_path):
        seen: dict = {}
    ...
    _d0 = tmp_path / "test_claude_driver_falls_back_to_transcript"
    _d0.mkdir()
    with pytest.MonkeyPatch.context() as mp:
        test_claude_driver_falls_back_to_transcript(_d0, mp)
    _d1 = tmp_path / "test_claude_driver_writes_mcp_config_and_cmd_flags"
    _d1.mkdir()
    test_claude_driver_writes_mcp_config_and_cmd_flags(_d1)
    _d2 = tmp_path / "test_claude_driver_server_command_override"
    _d2.mkdir()
    test_claude_driver_server_command_override(_d2)
    test_claude_driver_behaviours()
    _d4 = tmp_path / "test_claude_driver_uses_proxy_in_mcp_config"
    _d4.mkdir()
    test_claude_driver_uses_proxy_in_mcp_config(_d4)
    _d5 = tmp_path / "test_claude_driver_proxy_disabled_no_wrap"
    _d5.mkdir()
    test_claude_driver_proxy_disabled_no_wrap(_d5)
    _d6 = tmp_path / "test_claude_mcp_env_has_pythonpath_when_proxied"
    _d6.mkdir()
    test_claude_mcp_env_has_pythonpath_when_proxied(_d6)


def test_known_drivers_behaviours():
    ...
    test_known_drivers()
    test_known_drivers_and_get_driver()
...
def test_antigravity_behaviours(tmp_path):
    def test_antigravity_driver_writes_mcp_config_under_isolated_home(tmp_path):
        seen: dict = {}
    ...
    def test_antigravity_fallback_runner_timeout_harvests(tmp_path):
        call_row = {
            "tool": "g_tool",
            "args": {},
            "is_error": False,
    ...
    _d0 = tmp_path / "test_antigravity_driver_writes_mcp_config_under_isolated_home"
    _d0.mkdir()
    test_antigravity_driver_writes_mcp_config_under_isolated_home(_d0)
    _d1 = tmp_path / "test_antigravity_fallback_runner_timeout_harvests"
    _d1.mkdir()
    test_antigravity_fallback_runner_timeout_harvests(_d1)


def test_write_behaviours(tmp_path):
    def test_write_antigravity_mcp_config_shape(tmp_path):
    ...
    def test_write_opencode_mcp_config_shape(tmp_path):
    ...
    _d0 = tmp_path / "test_write_antigravity_mcp_config_shape"
    _d0.mkdir()
    test_write_antigravity_mcp_config_shape(_d0)
    _d1 = tmp_path / "test_write_opencode_mcp_config_shape"
    _d1.mkdir()
    test_write_opencode_mcp_config_shape(_d1)
