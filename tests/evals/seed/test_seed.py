...
def test_seed_behaviours(monkeypatch):
    def test_seed_plan_behaviours():
        def test_seed_plan_covers_all_groups():
            ...
            for g in groups:
                assert (
                    g.split("_")[0] in blob
                    or g in blob
                    or g.replace("_", " ") in blob
                    or any(g in line for line in lines)
        ...
        test_seed_plan_covers_all_groups()
        test_seed_plan_empty_needs_only_project()
    ...
    test_seed_plan_behaviours()
    test_seed_module_ast_has_all_group_handlers()
    with pytest.MonkeyPatch.context() as mp:
        test_seed_enables_project_features_immediately_after_create(mp)
    with pytest.MonkeyPatch.context() as mp:
        test_seed_s5_leaves_cycles_worklogs_and_customers_off(mp)
    with pytest.MonkeyPatch.context() as mp:
        test_seed_cycles_create_add_then_backdate(mp)
    with pytest.MonkeyPatch.context() as mp:
        test_seed_enables_features_on_second_project_too(mp)
...
def test_teardown_behaviours():
    ...
    test_teardown_leaves_workspace_alone_when_the_prior_value_is_unknown()
    test_teardown_deletes_release_tag_and_customer_property()
...
def test_preclean_behaviours():
    ...
    test_preclean_removes_stale_tag_and_property()
    test_preclean_delete_failure_raises_for_infra_seed()
    test_preclean_empty_list_is_silent()


def test_l2_activity_behaviours():
    ...
    test_l2_activity_gate_raises_when_empty()
    test_l2_activity_gate_proceeds_when_nonempty()


def test_create_behaviours(monkeypatch):
    ...
    with pytest.MonkeyPatch.context() as mp:
        test_create_project_retries_409_then_succeeds(mp)
    with pytest.MonkeyPatch.context() as mp:
        test_create_project_raises_after_max_409s(mp)
    test_create_project_non_collision_error_does_not_retry()
...
def test_cleanup_behaviours(monkeypatch, capsys):
    ...
    with pytest.MonkeyPatch.context() as mp:
        test_cleanup_dry_run_never_calls_delete(mp, capsys)
    with pytest.MonkeyPatch.context() as mp:
        test_cleanup_yes_deletes(mp, capsys)


def test_list_projects_behaviours():
    ...
    test_list_projects_with_prefix_filters()
    test_list_projects_two_page_pagination()
