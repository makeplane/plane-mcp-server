...
def test_single_rep_summary_renders_tool_distribution_unavailable(capsys):
    ...
    assert capsys.readouterr().out == (
        ...
        "  zero-call success: 0\n"
        "  write without a write call: not evaluated — this file declares no task tags, "
        "so mutation intent is unknown\n"
        "  answer without provenance: 0\n"
        "  implausibly few calls: 0; rule: among at least 5 successful trace-usable repetitions for the same "
        "task, calls < Q1 - 3×IQR and calls ≤ half the task median\n"
        "  limitation: detects off-surface work only when it leaves a trace signature; it cannot detect an agent "
        "that performs the work off-surface and also makes convincing surface calls\n"
        "schema friction (same successful, trace-intact rows as call deltas): task-mean median errored calls=0.0 "
        "across 1 tasks; task-mean errored-call rate=0.0% across 1 tasks with calls\n"
        "  errored-call tasks: 0/1 []\n"
        "  by kind, of 2 calls: surface friction=0 (0.0%), navigation=0 (0.0%), "
        "answered existence questions=0 (0.0%), other=0 (0.0%), unclassified=0 (0.0%)\n"
        "  surface friction is the number to act on: a well-formed call the API refused on "
        "meaning \u2014 none in this run\n"
        "  limitation: is_error is the MCP-level error flag, so this counts tool-reported failures; an error that is "
        "the correct task outcome still contributes, while calling the wrong tool successfully does not\n"
        "  limitation: a first not_found is read as the answer to an existence question, since asking has no "
        "cheaper form; only a repeat on the same tool and action is counted as friction. A surface that "
        "misleads an agent into one wrong lookup is therefore not charged for it\n"
        "POWER: 1 of 1 task(s) below 5 repetitions (fewest 1) \u2014 their per-task pass rates "
        "and UNSTABLE flags are not verdicts at that depth; read the aggregate and paired "
        "deltas for those. Raising --reps narrows a per-task interval but no fixed count "
        "makes one conclusive.\n"
