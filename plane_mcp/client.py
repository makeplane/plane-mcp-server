...
def get_plane_client_context() -> PlaneClientContext:
    ...
    if not stored_access_token:
        raise ConfigurationError(
            "No access token available. Authentication required. Provide one of: "
            "PLANE_API_KEY + PLANE_WORKSPACE_SLUG (env vars), "
            "x-api-key + x-workspace-slug (HTTP headers), "
            "or valid OAuth access token"
        )
    ...
    client = PlaneClient(**client_kwargs)
