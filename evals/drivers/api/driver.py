"""The owned model/tool loop: run one task through a registered API backend.

Unlike a CLI driver, this driver *is* the loop — it lists the MCP tool surface, asks the
backend for a turn, executes the calls itself and records each result as it comes back. So
it needs no recording proxy and no subprocess supervision: the evidence is in hand.

Provider differences live behind ``ModelBackend`` in ``backend.py``; nothing here knows
...
class ApiDriver:
    ...
    def __init__(
        ...
        *,
        ...
        backend_factory: Callable[[str, int], ModelBackend] | None = None,
