...
def stdio_server_env(*, surface: str = "full", extra: dict[str, str] | None = None) -> dict[str, str]:
    ...
    if surface == "v2":
        environment["PLANE_MCP_SURFACE"] = "v2"
    elif surface == "v2-schema":
        environment["PLANE_MCP_SURFACE"] = "v2-schema"
    if extra:
                    ...
                    if agent is not None:
                        ...
                        if driver_name.endswith("-cli") and is_infra_cli_stop_reason(
                            ...
                            try:
                                ...
                                print(
                                    f"  {task['id']} rep={repetition} success={ok} "
...
async def run_live(
    ...
    *,
    ...
    print(
        ...
        f"surface={surface} tasks={[task['id'] for task in tasks]} reps={reps}"
    )
    ...
    with out_path.open("a", encoding="utf-8") as file:
        for task in tasks:
            ...
            for repetition in range(reps):
                if (task["id"], repetition) in resume_skip:
